...
for Delta in Delta_values:
    ...
    R_UCB[i], _ = UCB1Algorithm(n, bandit, delta).regret_analysis(1000, Deltas)
    R_ETC_optimal[i], _ = ETCAlgorithm(n, optimal_m, bandit).regret_analysis(1000, Deltas)
    R_ETC_25[i], _ = ETCAlgorithm(n, 25, bandit).regret_analysis(1000, Deltas)
    R_ETC_50[i], _ = ETCAlgorithm(n, 50, bandit).regret_analysis(1000, Deltas)
    R_ETC_75[i], _ = ETCAlgorithm(n, 75, bandit).regret_analysis(1000, Deltas)
    R_ETC_100[i], _ = ETCAlgorithm(n, 100, bandit).regret_analysis(1000, Deltas)
    print(R_UCB[i])
    i += 1
...
plt.legend(['UBC', 'ETC, optimal', 'ETC, m = 25', 'ETC, m = 50', 'ETC, m = 75', 'ETC, m = 100'])
plt.grid()
plt.show()